# BListNotes.py
import tkinter as tk
from tkinter import messagebox
import Users_db
from BNote import Note


class ListNotes:
    def __init__(self, parent_wnd, user_id):
        self._parent_wnd = parent_wnd
        self.user_id = user_id

        self._this_wnd = tk.Toplevel(parent_wnd) if parent_wnd else tk.Tk()
        self._this_wnd.title("Your notes")

        self._note_window = None

        self.create_ui()


    def create_new(self):
        self._note_window = Note(
            self._this_wnd,
            self.user_id,
            on_saved=self.refresh_notes
        )

    def open_note(self, note_id):
        self._note_window = Note(
            self._this_wnd,
            self.user_id,
            note_id=note_id,
            on_saved=self.refresh_notes
        )

    def delete_note(self, note_id, title):
        ok = Users_db.delete_note(note_id, self.user_id)

        if ok:
            messagebox.showinfo("DELETE", f"Deleted:\n{title}", parent=self._this_wnd)
            self.refresh_notes()
        else:
            messagebox.showerror("DELETE", "Could not delete note", parent=self._this_wnd)

    def go_back(self):
        self._this_wnd.destroy()


    def create_ui(self):
        self._this_wnd.state("zoomed")
        self._this_wnd.resizable(True, True)

        self.BG = "#0b0b0f"
        self.PANEL = "#11111a"
        self.TEXT = "#e6e6eb"
        self.MUTED = "#9aa0aa"
        self.CYAN = "#22d3ee"
        self.PURPLE = "#a855f7"
        self.BORDER = "#1f2233"

        self.BTN_BG = "#2a2f3a"
        self.BTN_HOVER = "#343a46"
        self.BTN_TEXT = "#ffffff"
        self.BTN_BORDER = "#ffffff"

        self._this_wnd.configure(bg=self.BG)

        self._canvas = tk.Canvas(
            self._this_wnd,
            bg=self.BG,
            highlightthickness=0,
            bd=0
        )
        self._canvas.pack(fill="both", expand=True)

        self.scroll_canvas = tk.Canvas(
            self._canvas,
            bg=self.PANEL,
            highlightthickness=0,
            bd=0
        )

        self.scrollbar = tk.Scrollbar(
            self._this_wnd,
            orient="vertical",
            command=self.scroll_canvas.yview
        )

        self.scroll_canvas.configure(yscrollcommand=self.scrollbar.set)



        self.list_frame = tk.Frame(self.scroll_canvas, bg=self.PANEL)

        self.list_window_id = self.scroll_canvas.create_window(
            (0, 0),
            window=self.list_frame,
            anchor="nw"
        )

        self.list_frame.bind("<Configure>", self._on_list_frame_configure)
        self.scroll_canvas.bind("<Configure>", self._on_scroll_canvas_configure)
        self.scroll_canvas.bind_all("<MouseWheel>", self._on_mousewheel)

        self.list_frame.grid_columnconfigure(0, weight=1)
        self.list_frame.grid_columnconfigure(1, weight=0)

        self._btn_back = self._make_main_button(
            text="BACK",
            command=self.go_back
        )

        self._btn_new = self._make_main_button(
            text="CREATE NEW NOTE",
            command=self.create_new
        )

        self._canvas.bind("<Configure>", self._redraw_layout)

        self.refresh_notes()

        self._this_wnd.after(50, self._redraw_layout)



    def refresh_notes(self):
        for widget in self.list_frame.winfo_children():
            widget.destroy()

        Users_db.ensure_db_notes()
        notes = Users_db.get_user_notes(self.user_id)

        if not notes:
            empty_label = tk.Label(
                self.list_frame,
                text="No notes yet",
                bg=self.PANEL,
                fg=self.MUTED,
                font=("Calibri", 16)
            )
            empty_label.grid(row=0, column=0, padx=10, pady=20, sticky="w")
        else:
            for row, note in enumerate(notes):
                note_id = note[0]
                title = note[1]
                created_at = note[2]

                note_btn = self._make_list_button(
                    text=f"{title}   |   {created_at}",
                    command=lambda n_id=note_id: self.open_note(n_id)
                )
                note_btn.grid(row=row, column=0, padx=(10, 8), pady=6, sticky="ew")

                delete_btn = self._make_delete_button(
                    text="DELETE",
                    command=lambda n_id=note_id, t=title: self.delete_note(n_id, t)
                )
                delete_btn.grid(row=row, column=1, padx=(0, 10), pady=6)

        self._this_wnd.update_idletasks()
        self.scroll_canvas.configure(scrollregion=self.scroll_canvas.bbox("all"))


    def _make_main_button(self, text, command):
        btn = tk.Button(
            self._this_wnd,
            text=text,
            font=("Calibri", 12, "bold"),
            fg=self.BTN_TEXT,
            bg=self.BTN_BG,
            activeforeground=self.BTN_TEXT,
            activebackground=self.BTN_HOVER,
            bd=1,
            relief="solid",
            highlightthickness=1,
            highlightbackground=self.BTN_BORDER,
            highlightcolor=self.BTN_BORDER,
            command=command,
            cursor="hand2"
        )

        btn.bind("<Enter>", lambda e: e.widget.config(bg=self.BTN_HOVER))
        btn.bind("<Leave>", lambda e: e.widget.config(bg=self.BTN_BG))

        return btn

    def _make_list_button(self, text, command):
        btn = tk.Button(
            self.list_frame,
            text=text,
            font=("Calibri", 14, "bold"),
            fg=self.BTN_TEXT,
            bg=self.BTN_BG,
            activeforeground=self.BTN_TEXT,
            activebackground=self.BTN_HOVER,
            bd=1,
            relief="solid",
            highlightthickness=1,
            highlightbackground=self.BTN_BORDER,
            highlightcolor=self.BTN_BORDER,
            command=command,
            cursor="hand2",
            anchor="w",
            padx=12
        )

        btn.bind("<Enter>", lambda e: e.widget.config(bg=self.BTN_HOVER))
        btn.bind("<Leave>", lambda e: e.widget.config(bg=self.BTN_BG))

        return btn

    def _make_delete_button(self, text, command):
        btn = tk.Button(
            self.list_frame,
            text=text,
            font=("Calibri", 14, "bold"),
            fg=self.BTN_TEXT,
            bg=self.BTN_BG,
            activeforeground=self.BTN_TEXT,
            activebackground=self.BTN_HOVER,
            bd=1,
            relief="solid",
            highlightthickness=1,
            highlightbackground=self.BTN_BORDER,
            highlightcolor=self.BTN_BORDER,
            command=command,
            cursor="hand2",
            width=8
        )

        btn.bind("<Enter>", lambda e: e.widget.config(bg=self.BTN_HOVER))
        btn.bind("<Leave>", lambda e: e.widget.config(bg=self.BTN_BG))

        return btn


    def _redraw_layout(self, event=None):
        self._canvas.delete("all")

        width = self._canvas.winfo_width()
        height = self._canvas.winfo_height()

        if width <= 1 or height <= 1:
            return

        self._draw_grid(width, height)

        panel_w = 1160
        panel_h = 520

        panel_x1 = (width - panel_w) // 2  #380
        panel_y1 = (height - panel_h) // 2  #280
        panel_x2 = panel_x1 + panel_w
        panel_y2 = panel_y1 + panel_h

        self._canvas.create_rectangle(
            panel_x1 + 6,
            panel_y1 + 6,
            panel_x2 + 6,
            panel_y2 + 6,
            fill="#07070b",
            outline=""
        )

        self._canvas.create_rectangle(
            panel_x1,
            panel_y1,
            panel_x2,
            panel_y2,
            fill=self.PANEL,
            outline=self.BORDER,
            width=2
        )

        self._canvas.create_line(
            panel_x1,
            panel_y1,
            panel_x2,
            panel_y1,
            fill=self.CYAN,
            width=3
        )

        self._canvas.create_text(
            (panel_x1 + panel_x2) // 2,
            panel_y1 + 40,
            text="NOTES",
            font=("Calibri", 26, "bold"),
            fill=self.TEXT
        )

        self._canvas.create_text(
            (panel_x1 + panel_x2) // 2,
            panel_y1 + 70,
            text="keep your notes here",
            font=("Calibri", 14),
            fill=self.MUTED
        )

        list_x1 = panel_x1 + 40
        list_y1 = panel_y1 + 110
        list_x2 = panel_x2 - 40
        list_y2 = panel_y2 - 60

        list_w = list_x2 - list_x1
        list_h = list_y2 - list_y1

        self.scroll_canvas.place(
            x=list_x1,
            y=list_y1,
            width=list_w,
            height=list_h
        )

        self.scrollbar.place(
            x=list_x2 + 8,
            y=list_y1,
            height=list_h
        )

        self._canvas.create_text(
            (panel_x1 + panel_x2) // 2,
            panel_y2 - 28,
            text="tip: click NEW to create a note",
            font=("Calibri", 11),
            fill="#7e8593"
        )

        self._btn_back.place(x=100, y=120, width=100, height=40)
        self._btn_new.place(x=220, y=120, width=300, height=40)

        self._btn_back.lift()
        self._btn_new.lift()

        self._this_wnd.update_idletasks()
        self.scroll_canvas.configure(scrollregion=self.scroll_canvas.bbox("all"))

    def _draw_grid(self, width, height):
        step = 45

        for x in range(0, width, step):
            self._canvas.create_line(
                x,
                0,
                x,
                height,
                fill="#2D3458",
                tags="grid"
            )

        for y in range(0, height, step):
            self._canvas.create_line(
                0,
                y,
                width,
                y,
                fill="#2D3458",
                tags="grid"
            )

        self._canvas.tag_lower("grid")


    def _on_list_frame_configure(self, event):
        self.scroll_canvas.configure(
            scrollregion=self.scroll_canvas.bbox("all")
        )

    def _on_scroll_canvas_configure(self, event):
        self.scroll_canvas.itemconfig(
            self.list_window_id,
            width=event.width
        )

    def _on_mousewheel(self, event):
        self.scroll_canvas.yview_scroll(
            int(-1 * (event.delta / 120)),
            "units"
        )


    def show_modal(self):
        self._this_wnd.grab_set()

    def run(self):
        if self._parent_wnd is None:
            self._this_wnd.mainloop()